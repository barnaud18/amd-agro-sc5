...
var leads = mysqlTable("leads", {
  id: int("id").autoincrement().primaryKey(),
  produto: varchar("produto", { length: 32 }).notNull(),
  // "sc5" | "zmgrow"
...
async function getPendingLeads() {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return db.select().from(leads).where(eq(leads.googleSheetsSync, "pending"));
}
async function markLeadSynced(id, status) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(leads).set({ googleSheetsSync: status }).where(eq(leads.id, id));
}
...
var GOOGLE_SHEETS_WEBHOOK = process.env.GOOGLE_SHEETS_WEBHOOK_URL ?? "";
...
var leadsRouter = router({
  submit: publicProcedure.input(
    z2.object({
      produto: z2.enum(["sc5", "zmgrow"]),
      nome: z2.string().min(2),
      whatsapp: z2.string().min(8),
      cultura: z2.string().optional(),
      hectares: z2.string().optional(),
      problema: z2.string().optional()
    })
  ).mutation(async ({ input }) => {
    await insertLead({
...
Problema: ${input.problema ?? "\u2014"}`
  ...
  list: adminProcedure.input(
    z2.object({
      produto: z2.enum(["sc5", "zmgrow", "all"]).optional().default("all")
    })
  ).query(async ({ input }) => {
